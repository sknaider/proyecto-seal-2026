...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'mcp_session_agent() definition drifted; refusing rewrite';
    END IF;
    IF EXISTS (
        SELECT 1
          FROM pg_proc proc,
    ...
    IF topology IS NULL OR topology NOT IN ('*', 'r') THEN
        RAISE EXCEPTION
            'unexpected mcp_hard_identity topology';
    END IF;
    ...
    LOOP
        ...
        IF topology = '*' THEN
            ...
            THEN
                RAISE EXCEPTION
                    'unexpected legacy policy; refusing rewrite';
            END IF;
        ELSE
            CASE policy_row.polname
                WHEN 'mcp_hard_identity' THEN
                    IF policy_row.polcmd <> 'r'
                       ...
                       OR normalized_check <> ''
                    THEN
                        RAISE EXCEPTION 'unexpected SELECT policy';
                    END IF;
                WHEN 'mcp_hard_identity_insert' THEN
                    ...
                    THEN
                        RAISE EXCEPTION 'unexpected INSERT policy';
                    END IF;
                WHEN 'mcp_hard_identity_update' THEN
                    ...
                    THEN
                        RAISE EXCEPTION 'unexpected UPDATE policy';
                    END IF;
                WHEN 'mcp_hard_identity_delete' THEN
                    IF policy_row.polcmd <> 'd'
                       ...
                       OR normalized_check <> ''
                    THEN
                        RAISE EXCEPTION 'unexpected DELETE policy';
                    END IF;
            END CASE;
        END IF;
    END LOOP;
...
USING (
    agent IS NULL
    OR agent::text = 'TEAM'
    OR agent::text = soul_v3.mcp_session_agent()
);

CREATE POLICY mcp_hard_identity_insert
ON soul_v3.rules
AS RESTRICTIVE
FOR INSERT
TO mcp_runtime_ada, mcp_runtime_alice, mcp_runtime_dum,
...
CREATE POLICY mcp_hard_identity_update
ON soul_v3.rules
AS RESTRICTIVE
FOR UPDATE
TO mcp_runtime_ada, mcp_runtime_alice, mcp_runtime_dum,
...
CREATE POLICY mcp_hard_identity_delete
ON soul_v3.rules
AS RESTRICTIVE
FOR DELETE
TO mcp_runtime_ada, mcp_runtime_alice, mcp_runtime_dum,
