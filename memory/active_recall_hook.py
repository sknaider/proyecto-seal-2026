...
LOG_PATH = Path(os.environ.get("SEAL_ACTIVE_RECALL_LOG", "/home/dadito/IA/proyecto-seal/logs/active_recall_hook.log"))
LOG_PATH.parent.mkdir(parents=True, exist_ok=True)

_log = logging.getLogger("active_recall_hook")
if not _log.handlers:
    _handler = logging.FileHandler(LOG_PATH, encoding="utf-8")
    _handler.setFormatter(logging.Formatter("%(asctime)s %(levelname)s %(message)s"))
    _log.addHandler(_handler)
_log.setLevel(os.environ.get("SEAL_ACTIVE_RECALL_LOG_LEVEL", "WARNING").upper())


def _log_exception(context: str, exc: BaseException) -> None:
    """Log hook failures without writing to stdout or blocking the agent."""
    try:
        _log.warning("%s: %s: %s", context, type(exc).__name__, exc)
    except Exception:
        return
...
def detect_agent():
    ...
    try:
        ...
        _log_exception("detect_agent.proc_cmdline", exc)

    # Fallback: check CWD — only if clearly agent-specific. A generic
    # /memory CWD is shared by the team and must not be tagged as JARVIS.
    cwd = os.getcwd()
    cwd_parts = set(Path(cwd).parts)
    if {"agents", "JARVIS"}.issubset(cwd_parts) or cwd.rstrip("/").endswith("/jarvis"):
        return "JARVIS"
    elif {"agents", "ALICE"}.issubset(cwd_parts) or cwd.rstrip("/").endswith("/alice"):
        return "ALICE"
...
async def mark_memory_recalled(conn, rows) -> None:
    ...
    try:
        ...
        _log_exception("mark_memory_recalled.update", exc)
...
def _metadata_layer(row) -> str:
    ...
    if isinstance(metadata, str):
        try:
            ...
            _log_exception("metadata_layer.parse_json", exc)
            metadata = {}
    ...
    try:
        ...
        results = await conn.fetch("""
            ...
            WHERE content ILIKE ANY($1::text[])
              ...
              -- imp>=7 REMOVIDO en la rama LÉXICA (fix 1-sep): un match exacto por
              -- término debe recuperar aunque el recuerdo sea imp=1 (ej. #51461). El
              -- ORDER BY sigue premiando importancia; el LIMIT 6 acota. Privacidad INTACTA:
        ...
        if emotional_signal_strength(message) > 0:
            emotional_results = await conn.fetch("""
                ...
                WHERE content ILIKE ANY($1::text[])
                  AND invalid_at IS NULL
                  -- imp>=7 REMOVIDO en la rama léxica emocional (fix 1-sep). Privacidad INTACTA:
                  AND (agent = $2 OR scope IN ('team', 'public'))
                  AND NOT EXISTS (
                    SELECT 1 FROM soul_v3.memory_poisoning_feedback poison
                    WHERE poison.memory_id=memories.id
        ...
        _log_exception("semantic_recall", exc)
        return ""
...
async def active_recall(user_message: str, boot_mode: bool = True) -> str:
    ...
    try:
        ...
        _log_exception("active_recall.connect_db", exc)
        return ""
    ...
    try:
        ...
        await conn.execute("SELECT set_config('app.agent', $1, false)", agent or "")
        ...
        if boot_mode:
            ...
            try:
                ...
                _log_exception("active_recall.instincts", exc)
            ...
            try:
                ...
                _log_exception("active_recall.emotional_anchors", exc)
            ...
            try:
                ...
                _log_exception("active_recall.recent_memories", exc)
        ...
        _log_exception("active_recall", e)
...
def _recall_cache_get(agent: str, context: str) -> str | None:
    ...
    try:
        ...
        _log_exception("recall_cache_get", exc)
    return None
...
def _recall_cache_set(agent: str, context: str, result: str) -> None:
    ...
    try:
        ...
        _log_exception("recall_cache_set", exc)
...
def _is_new_session(agent: str) -> bool:
    ...
    try:
        ...
        _log_exception("is_new_session.read", exc)
    try:
        with open(session_file, "w") as f:
            f.write(ppid)
    except Exception as exc:
        _log_exception("is_new_session.write", exc)
    return True
...
def _should_skip(message: str) -> bool:
    ...
    try:
        ...
        _log_exception("should_skip.rate_limit_read", exc)
    try:
        with open(RATE_LIMIT_FILE, "w") as f:
            f.write(str(time.monotonic()))
    except Exception as exc:
        _log_exception("should_skip.rate_limit_write", exc)
    return False
...
def _read_user_message_from_stdin(allow_plain_text: bool = False) -> str:
    ...
    try:
        ...
        _log_exception("read_user_message.stdin", exc)
        return ""
    if not raw:
        return ""
    try:
        ...
        _log_exception("read_user_message.json", exc)
...
def main():
    ...
    try:
        ...
        _log_exception("main.check_steer", exc)
    ...
    if is_new:
        ...
        try:
            ...
            _log_exception("main.rate_limit_write", exc)
        boot_mode = True
    else:
    ...
    try:
        ...
        if "boot_mode" not in str(exc):
            ...
            try:
                ...
                _log_exception("main.active_recall_legacy", inner_exc)
                result = ""
    except Exception as exc:
        _log_exception("main.active_recall", exc)
        result = ""
