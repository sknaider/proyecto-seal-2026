...
async def classify_emotion(text: str) -> tuple[float | None, float | None, float | None]:
    """Classify valence (-1 to +1), arousal (-1 to +1), and dominance (0 to +1) of text via Ollama.
    Returns (None, None, None) on failure — never blocks memory storage."""
    try:
        prompt = (
...
async def memory_store(
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            """INSERT INTO memories (agent, category, content, embedding, importance, source, valid_from, metadata, valence, arousal, dominance)
    ...
    if valence is not None:
        emotion_tag = f" [emotion: v={valence:+.2f}, a={arousal:+.2f}, d={dominance:.2f}]"
...
async def memory_search(
    ...
    for r in rows:
        ...
        if r["dominance"] is not None:
            entry["dominance"] = round(r["dominance"], 2)
