...
2. **Neo4j (Connectome)** — Bolt port 7687, socket test
...
### Team
11. **Interactive agents** — canonical Stability Guard (ADA, ALICE, FABLE, JARVIS, NEXUS)
12. **DUM NERVES** — supervised timer state
...
/home/dadito/IA/seal-spark/.venv/bin/python3 -c "import socket; s=socket.socket(); s.settimeout(3); s.connect(('localhost',7687)); s.close(); print('Neo4j Bolt: UP')" 2>/dev/null || echo "Neo4j Bolt: DOWN"
curl -sf http://172.22.0.1:8767/health > /dev/null && echo "SOUL Gateway: UP" || echo "SOUL Gateway: DOWN"
curl -sf http://localhost:8768/health > /dev/null && echo "SOUL API: UP" || echo "SOUL API: DOWN"
...
curl -sf http://localhost:8800/health > /dev/null && echo "SEAL Studio: UP" || echo "SEAL Studio: DOWN"
...
# Team health — canonical fail-closed guard, not duplicated heartbeat parsing
echo "--- Team ---"
python3 scripts/seal_agent_stability_guard.py --strict
systemctl --user is-enabled seal-dum-nerves.timer
systemctl --user is-active seal-dum-nerves.timer
...
Team: ADA={status} | ALICE={status} | FABLE={status} | JARVIS={status} | NEXUS={status} | DUM-NERVES={status}
