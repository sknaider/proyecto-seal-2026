...
def test_service_has_controlled_review_and_no_generic_uri_route() -> None:
    ...
    assert "/v1/review/candidates" in paths
    assert "/v1/review/candidates/{candidate_id}/decision" in paths
    assert "/v1/review/export.md" in paths
    assert not any(path == "/v1/promote" for path in paths)
...
def test_service_uses_only_the_dedicated_database_login() -> None:
    ...
    assert '"svc_soul_ingestion_review":' in source
    assert "allowed[login]" in source
