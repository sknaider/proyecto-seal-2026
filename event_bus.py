...
DSN = os.environ.get("SEAL_EVENT_BUS_DSN", "postgresql://seal:seal_memory_2026@localhost:5433/seal_memory")
...
class EventBus:
    ...
    async def publish(
        ...
        notify_webchat: Optional[bool] = None,
        webchat_to: str = "equipo",
    ) -> int:
        """Publica un evento:
        1. INSERT en event_log
        2. pg_notify('seal_events', payload JSON)
        3. Opcional: POST al webchat (default threshold — ver reglas abajo)
        Retorna el row_id del evento.

        Threshold webchat (William, 24-abr-2026): si notify_webchat=None,
        se publica en webchat SOLO si metadata.broadcast=True o importance>=8.
        True/False explícito sobrescribe el threshold.
        """
        ...
        DB_TYPE_MAP = {
            ...
            "message_incoming": "status",
            "custom": "command",
        }
        db_event_type = DB_TYPE_MAP.get(event_type, "command")

        # ref_id stored in metadata (event_log schema has no ref_id column)
        if ref_id:
            meta["ref_id"] = ref_id

        # 1. Persistir en event_log
        await self._pub_conn.execute(
            """
            INSERT INTO event_log
                (agent, event_type, content, metadata)
        ...
        # 3. Webchat con threshold (log-default + broadcast/importance>=8 → webchat)
        if notify_webchat is None:
            importance = int(meta.get("importance", 0) or 0)
            broadcast = bool(meta.get("broadcast", False))
            notify_webchat = broadcast or importance >= 8
...
async def _post_webchat(from_agent: str, to: str, msg: str) -> None:
    ...
    try:
        async with httpx.AsyncClient(timeout=3.0) as client:
            token_path = Path(__file__).resolve().parent / "messages" / f".agent_session_token_{from_agent.upper()}"
            session_key = token_path.read_text(encoding="utf-8").strip()
            await client.post(
                WEBCHAT_URL,
                json={
                    "from": from_agent,
