...
for AGENT in JARVIS ADA ALICE NEXUS; do
...
for AGENT in JARVIS ADA ALICE DUM NEXUS; do
...
for NAME in "JARVIS" "ADA" "ALICE" "NEXUS"; do
...
# 3.5 Matar mcp_server.py zombies (hijos reparentados a init tras matar claude)
...
MCP_PIDS=$(pgrep -f "mcp_server.py" 2>/dev/null)
if [ -n "$MCP_PIDS" ]; then
  KILLED=0
