...
EXPECTED = {
    ...
    # bench_runs NO es un daemon: se escribe MANUAL o por safety_governor (verificado por efecto
    # 20-jun: triggered_by manual_v3/v4/v5/safety_governor, gaps irregulares 22h→273h, SIN timer).
    # Umbral 240h daba falso "daemon cayó en silencio" (no hay daemon). 720h = 30d = solo alerta si
    # el bench quedó VERDADERAMENTE abandonado (el keystone "no se mide"), no por cadencia normal.
    # FIX REAL pendiente (ADA): agendar el bench en timer → se vuelve daemon de verdad y baja el umbral.
    "bench_runs":          (720, "ADA"),
}
