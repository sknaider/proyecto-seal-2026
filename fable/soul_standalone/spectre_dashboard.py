...
RELEASE_PRODUCTS = {
    ...
    "platform": {
        ...
        "repo": "proyecto-seal-2026",
        "tag_pattern": r"soul-platform-v(?P<version>\d+\.\d+\.\d+)-\d{8}",
        "asset_template": "SOUL-Platform-{version}-Windows.zip",
        "asset_suffix": ".zip",
        "fallback": "https://github.com/sknaider/proyecto-seal-2026/releases",
        "private": True,
    },
}
...
def _release_metadata(product, payload):
    ...
    valid_release = not payload.get("draft") and bool(match)
    if product == "core":
        valid_release = valid_release and not payload.get("prerelease")
...
def download_release_asset(product):
    """Download a private release asset with server credentials and verify exact bytes."""
    metadata = latest_release(product)
    api_path = metadata.get("_asset_api_path")
    expected_digest = metadata.get("_asset_digest")
    expected_size = metadata.get("_asset_size")
    if not metadata.get("direct") or not api_path or not expected_digest or not expected_size:
        raise ValueError("No hay un instalador privado verificable disponible")
    completed = subprocess.run(
        ["gh", "api", api_path, "-H", "Accept: application/octet-stream"],
        capture_output=True,
        check=True,
        timeout=60,
    )
    ...
    if len(body) != expected_size or hashlib.sha256(body).hexdigest() != expected_digest:
        raise ValueError("El instalador descargado no coincide con la release firmada")
    return metadata["asset_name"], body
...
<div class=plinks><a id=coreInstaller class=release href="https://github.com/sknaider/soul-framework/releases/latest" target=_blank rel=noopener title="Descargar la última versión de SOUL Core">&#11015; Core básico · Descargar</a><a id=platformInstaller class=release href="https://github.com/sknaider/proyecto-seal-2026/releases" target=_blank rel=noopener title="Descargar la última versión de SOUL Platform">&#11015; Platform completo · Descargar</a><a href="https://pypi.org/project/soul-framework/" target=_blank rel=noopener>&#128230; PyPI</a><a href="https://github.com/sknaider/soul-framework/blob/main/docs/quickstart.md" target=_blank rel=noopener>&#128214; Docs</a><code>pip install soul-framework</code></div></div>
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if self.path == "/api/release-download/platform":
                try:
                    filename, body = download_release_asset("platform")
                    return self._download(filename, body)
                except (OSError, ValueError, subprocess.SubprocessError):
                    return self._deny(503, "Instalador privado no disponible o no verificable")
