...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "show":
        ...
        st = save_checkpoint(
            "Construir prototipo SOUL v2 al 100% + montar centro de operaciones externo (.70)",
            85,
            ["Fase 4: lector-arXiv probado por efecto",
             "Fase 4: sandbox probado 5 frentes",
             "Arquitectura aprobada por William: motor→.70, alma(DB)→.200 por LAN",
             "Fase 5: checkpoint de continuidad construido"],
            "Conseguir acceso PROPIO al .70 (no la llave de ALICE) → levantar Ollama + 3 modelos → apuntar el motor a .70")
