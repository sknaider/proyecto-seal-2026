...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.soul/.env ] && grep -q "^GITHUB_TOKEN=" ~/.soul/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.soul/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${SOUL_HOME:-$HOME/.soul}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by SEAL. Code looks clean — good test coverage, no security concerns."
