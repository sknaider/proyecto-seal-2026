...
metadata:
  soul:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, soul-video]
...
```
SEAL -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${SOUL_HOME:-$HOME/.soul}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to SEAL config (if missing)
...
3. **Restart SEAL session** to pick up the new MCP server
