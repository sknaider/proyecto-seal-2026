...
if _HAVE_ROS:
    class MazeNavigator(Node):
        def __init__(self):
            ...
            # ALICE 13-jul: STOP por teclado. El humano marca la META (vuelta_keyboard
            # publica Bool True en /meta_marcada). Al recibirlo, el robot para y queda
            # detenido = LOGRO 1 cumplido. No auto-detecta la meta por odom.
            self.meta_marcada = False
            self.sub_meta = self.create_subscription(
                Bool, "/meta_marcada", self.on_meta, 10)
            ...
            self.get_logger().info(
                ...
                f"odom={self.odom_topic} cmd={self.cmd_topic} report={self.report_path}")
        ...
        def on_meta(self, msg):
            # ALICE 13-jul: el humano marcó la META por teclado -> parar para siempre.
            try:
                if bool(msg.data):
                    self.meta_marcada = True
            except Exception:
                pass
        ...
        def on_odom(self, msg):
            ...
            self._warned_no_odom = False
            self._warned_odom_stale = False
            # FABLE loop-completion: feed raw pose; latches done after one full lap
        ...
        def tick(self):
            if self.scan is None:
                return
            p = self.p
            # ALICE 13-jul: META marcada por teclado -> parar y quedarse detenido (LOGRO 1 cumplido).
            if self.meta_marcada:
                if self.state != "META":
                    self.get_logger().info("🏁 META marcada por teclado — deteniendo. LOGRO cumplido.")
                    self.state = "META"
                self.publish(0.0, 0.0)
                return
            if self.state == "FAULT":
