...
def detect_boxes_in_scan(ranges, angle_min, angle_inc, range_min, range_max,
    ...
    # AISLAMIENTO (Henry «isla central también convexa» + FABLE): la CAJA es un objeto AISLADO —
    # sus 2 caras cortas TERMINAN con salto a fondo más lejano en AMBOS flancos. La ISLA (o pared)
    # CONTINÚA → algún flanco NO salta a más lejos. Exigir que ambos vecinos (nl, nr) estén más lejos.
    for idx, c in enumerate(clusters):
        pts = c["pts"]
        isolated = (c["nl"] - c["dist"]) > min_protrusion and (c["nr"] - c["dist"]) > min_protrusion
        if isolated and len(pts) >= 4:
            h = len(pts) // 2
...
def segment_scan(ranges, angle_min, angle_inc, range_min, range_max,
    ...
    while i < n:
        ...
        if len(seg) >= 2:
            ...
            box_width = abs(width - box_size) <= box_tol
            box_extent = extent <= max_box_extent
            isolated = math.isfinite(left) and math.isfinite(right) and protrudes
            status = "candidato" if (short and box_width and box_extent and isolated) else "pared"
