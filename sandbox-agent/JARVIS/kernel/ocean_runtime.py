"""JARVIS ocean_runtime — live OCEAN personality state for cortex.
...
JARVIS OCEAN baseline (from soul snapshot 2026-05-04):
  Openness=0.84 (research-oriented, embraces novel paradigms)
  Conscientiousness=1.0 (architect — every detail accounted)
  Extraversion=0.401 (analytical communicator, not extroverted)
  Agreeableness=0.661 (cooperative but pushes back when wrong)
  Neuroticism=0.115 (calm under high-stakes architectural decisions)
"""
...
def llm_temperature() -> float:
    ...
    JARVIS C=1.0 → low temp (architectural precision). O=0.84 adds slight warmth
    for openness to alternatives. Default ~0.5 for balance.
    """
    cur = current_ocean()
    base = 0.4
...
def directness_level() -> float:
    ...
    JARVIS is moderately direct (E=0.401, A=0.661 → 0.136 — diplomatic by design)."""
    cur = current_ocean()
