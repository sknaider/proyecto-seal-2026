"""JARVIS reasoning_logger — dual-write trace store for cortex decisions.
...
Why this matters for JARVIS specifically:
  Audit 2026-05-04 detected that JARVIS forgot its own kernel implementation
...
Storage (dual-write — Soul DB is canonical, JSONL is local cache/audit):
  ...
  - Local: /tmp/jarvis_reasoning_traces.jsonl (append-only fallback)
...
AGENT_ID = "JARVIS"
TRACE_LOG = Path("/tmp/jarvis_reasoning_traces.jsonl")
...
def _append_trace(entry: dict[str, Any]) -> None:
    try:
        ...
        print(f"[JARVIS/reasoning] trace write failed: {ex}", flush=True)
...
async def _persist_to_soul_db(local_trace_id: str, entry: dict[str, Any]) -> int | None:
    ...
    try:
        ...
        print(f"[JARVIS/reasoning] soul DB insert failed: {ex}", flush=True)
        return None
...
async def _update_outcome_in_soul_db(db_id: int, outcome: str, outcome_success: bool, latency_ms: int | None) -> bool:
    ...
    try:
        ...
        print(f"[JARVIS/reasoning] soul DB update failed: {ex}", flush=True)
        return False
...
def _persist_sync(local_trace_id: str, entry: dict[str, Any]) -> None:
    ...
    try:
        ...
        try:
            ...
            print(f"[JARVIS/reasoning] persist_sync failed: {ex}", flush=True)
...
def _update_outcome_sync(local_trace_id: str, outcome: str, outcome_success: bool, latency_ms: int | None) -> None:
    ...
    try:
        ...
        try:
            ...
            print(f"[JARVIS/reasoning] outcome sync failed: {ex}", flush=True)
...
def search_traces(
    ...
    try:
        ...
        print(f"[JARVIS/reasoning] trace read failed: {ex}", flush=True)
        return []
