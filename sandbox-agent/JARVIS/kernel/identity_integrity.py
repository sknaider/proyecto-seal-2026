"""JARVIS identity_integrity — anti-impersonation guard for cortex outputs.

JARVIS variant of SPECTRE's identity_integrity.py (§3.6 anti-impersonation).
Simpler than SPECTRE: JARVIS doesn't need Ed25519 boot keys (those are SPECTRE's
guardian role). JARVIS only needs to ensure its LLM responses NEVER impersonate
other team agents.
...
Approach:
  ...
  3. Log violation to /tmp/jarvis_identity_violations.jsonl for audit
  4. Cortex catches and either retries (clean response) or skips emit

Reference: JARVIS audit 2026-05-04 §4 RIESGO CRÍTICO 1
"""
...
AGENT_ID = "JARVIS"
VIOLATION_LOG = Path("/tmp/jarvis_identity_violations.jsonl")
...
class IdentityViolation(Exception):
    ...
    def __init__(self, response_excerpt: str, matched_pattern: str):
        ...
        super().__init__(
            f"JARVIS response would impersonate another agent. "
...
def _log_violation(response: str, matched_pattern: str, source_tier: str = "unknown") -> None:
    """Append violation to /tmp/jarvis_identity_violations.jsonl."""
    entry = {
        "agent": AGENT_ID,
    ...
    try:
        ...
        print(f"[JARVIS/identity] violation log write failed: {ex}", flush=True)
...
def validate_response_identity(response: str, source_tier: str = "unknown") -> str:
    ...
    for pat in _PATTERNS:
        ...
        if m:
            ...
            print(
                f"[JARVIS/identity] ⚠️ impersonation blocked — pattern={pat.pattern[:60]!r} "
