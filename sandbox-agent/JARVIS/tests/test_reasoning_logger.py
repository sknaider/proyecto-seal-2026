"""Tests for JARVIS reasoning_logger — local trace lifecycle + Soul DB dual-write."""
...
def test_store_trace_returns_uuid(isolated_log):
    tid = reasoning_logger.store_trace(
        task="strategize", input_excerpt="x", premises=["p"],
...
def test_store_trace_writes_to_log(isolated_log):
    reasoning_logger.store_trace(
        task="t", input_excerpt="x", premises=[], reasoning="r", decision="d",
    )
    assert isolated_log.exists()
    entry = json.loads(isolated_log.read_text().splitlines()[0])
    assert entry["agent"] == "JARVIS"
    assert entry["task"] == "t"
...
def test_store_trace_caps_long_strings(isolated_log):
    long = "x" * 5000
    reasoning_logger.store_trace(task="t", input_excerpt=long, premises=["p"],
                                  reasoning=long, decision=long)
    entry = json.loads(isolated_log.read_text().splitlines()[0])
...
def test_search_filter_by_task_prefix(isolated_log):
    reasoning_logger.store_trace(task="alpha_one", input_excerpt="x", premises=[],
                                  reasoning="r", decision="d")
    reasoning_logger.store_trace(task="beta_one", input_excerpt="x", premises=[],
                                  reasoning="r", decision="d")
...
# ── Soul DB dual-write contract (mocked) ──────────────────────────────────────
