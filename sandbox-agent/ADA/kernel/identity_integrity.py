"""ADA identity_integrity — anti-impersonation guard for cortex outputs.

ADA variant. Same architecture as ALICE/JARVIS/NEXUS:
- regex anti-impersonation, no Ed25519
- vocative ("William, ...") is legitimate 2nd-person speech, NOT impersonation
- only 1st-person patterns count: "[NAME]", "NAME:", "NAME dice:", "Soy NAME"

Fix vocative applied from day 1 (lesson from NEXUS bug 2026-05-04 13:50).
"""
...
AGENT_ID = "ADA"
VIOLATION_LOG = Path("/tmp/ada_identity_violations.jsonl")

OTHER_AGENTS = ("JARVIS", "ALICE", "NEXUS", "SPECTRE", "DUM", "William", "Kinger", "Henry")
...
class IdentityViolation(Exception):
    def __init__(self, response_excerpt: str, matched_pattern: str):
        ...
        super().__init__(
            f"ADA response would impersonate another agent. "
...
def _log_violation(response: str, matched_pattern: str, source_tier: str = "unknown") -> None:
    ...
    try:
        ...
        print(f"[ADA/identity] violation log write failed: {ex}", flush=True)
...
def validate_response_identity(response: str, source_tier: str = "unknown") -> str:
    ...
    for pat in _PATTERNS:
        if pat.match(response):
            ...
            print(
                f"[ADA/identity] ⚠️ impersonation blocked — pattern={pat.pattern[:60]!r} "
