...
- `research/flywire_results/nerves_orchestrator_inbox/JARVIS.state.json`: estado de orquestador de nerves que lee `memory/nerves_read_only_action_guard.py`; sin él, el guardián de ADA niega TODO (`mission_state_untrusted:FileNotFoundError`) y la dejó muda desde la mañana. Sin copia en NFS ni arenas. Repuesto por JARVIS con `deliveries` vacío y anotación `_reset` (estado real: ninguna misión A2 sobrevivió), 0600, fuera de git. El guardián falla cerrado por diseño; lo que faltó fue inventariar el archivo como estado esencial con respaldo.
