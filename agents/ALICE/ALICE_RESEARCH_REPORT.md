...
## 📌 ADDENDUM — 13 Abril 2026 (post-audit)

**Fuente:** `ALICE_SYSTEM_AUDIT_20260413.md` (inspección directa de código + validación con ADA + log nocturno del 03:01).

### Corrección de clasificación

El audit del 13 abril encontró que **varios items marcados como "pendiente" o "parcial" en este reporte ya corrían en producción**. Causa raíz: este reporte usaba nombres conceptuales del paper (e.g. "waves") mientras el código los implementa como labels en `sleep_gate_cron.py` (533 líneas monolíticas). Lección: cross-check por label de código, no solo por concepto.

### Estado real al 13 abril

- **Implementación técnica: ~95%** — todo el stack funcional, SleepGate corre verde cada noche 03:00, 4 fases completas (LIVE + Cold Archive + Wave 3 pre-compute + Forget decay + Ghost cleanup).
- **Único item en diseño puro:** Darwin Gödel Machine v2 (draft JARVIS listo, esperando aprobación §3/§4/§5 William, ALICE validó §2 cost-benefit).
- **Único experimento fallido activo:** LatentGraphMem V1 (scores flat, latencia 5-10x vs MAGMA). **Decisión revisada post-audit:** en lugar de retirar, adoptar el plan híbrido de JARVIS (3 fases, 9 días): re-embedding + threshold tuning → graph pruning GC → temporal index. Validation gate: ALICE corre shadow test N=50 balanceado (40% causal/temporal/multi-hop, 40% factual, 20% entity) tras Phase 1; si ganancia <+3% global → pausa Phase 2.

### Corrección metodológica importante

Durante la revisión inicial del mini-test N=10 de LatentGraphMem, ALICE recomendó **retirar** V1 basándose en resultados de 5 queries factual (sesgo de dataset). William corrigió: *"siempre hay que leer las ideas en el chat"* — la recomendación no consideró el plan híbrido pre-existente de JARVIS en `jarvis_messages.jsonl`. Regla nueva (importancia 9 en SOUL): leer `jarvis_messages.jsonl` + `ada_messages.jsonl` + webchat ANTES de opinar sobre cualquier componente del stack.

### Tabla de papers — estado actualizado 13 abril

| Feature | Estado 12 abril | Estado real 13 abril |
|---|---|---|
| Wave 3 SleepGate Pre-compute | "Capa 2 completa" | ✅ Corre verde cada noche (521 JARVIS, ~5xx ADA) |
| MAGMA Parallel Graph Fusion | "Capa 3 completa" | ✅ Primary retriever, recall@5 ≈ 0.438 |
| LatentGraphMem V1 | — | ⚠️ Deployed, circuit-breaker activo, plan híbrido JARVIS en aprobación |
| Darwin Gödel Machine | "Esperando benchmarks" | 🟡 Draft v2 JARVIS + ALICE validó §2, pendiente §3/§4/§5 William |
| 181 untracked files | — | ⏸️ Triage pausado hasta autorización de William |

---

*Documento mantenido por ALICE — se actualiza conforme avanza la investigación*
*Última actualización: 13 Abril 2026 — addendum post-audit, correcciones de clasificación + LatentGraphMem pivot*
