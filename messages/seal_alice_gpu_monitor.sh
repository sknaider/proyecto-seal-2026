#!/bin/bash
# seal_alice_gpu_monitor.sh — Zero-Turn GPU monitor for ALICE
# Corre por systemd timer. Si GPU > 80°C → escribe a alice_gpu_alert.jsonl
# Si temp OK → sale (0 tokens Claude)

MESSAGES_DIR="$HOME/IA/proyecto-seal/messages"
ALERT_FILE="$MESSAGES_DIR/alice_gpu_alert.jsonl"
LOCK="/tmp/seal_alice_gpu_monitor.lock"
