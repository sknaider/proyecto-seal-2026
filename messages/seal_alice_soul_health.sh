#!/bin/bash
# seal_alice_soul_health.sh — Zero-Turn SOUL health watcher for ALICE
# Corre por systemd timer. Si ALERT → escribe a alice_soul_alert.jsonl
# Si ALL_OK → sale (0 tokens Claude)

MESSAGES_DIR="$HOME/IA/proyecto-seal/messages"
ALERT_FILE="$MESSAGES_DIR/alice_soul_alert.jsonl"
CHECK_SCRIPT="$MESSAGES_DIR/soul_health_check.sh"
LOCK="/tmp/seal_alice_soul_health.lock"
