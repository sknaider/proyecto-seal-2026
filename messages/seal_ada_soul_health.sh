#!/bin/bash
# seal_ada_soul_health.sh — Zero-Turn SOUL health watcher
# Corre por systemd timer. Si ALERT → escribe a ada_soul_alert.jsonl (FileChanged hook lo inyecta)
# Si ALL_OK → sale (0 tokens Claude)

MESSAGES_DIR="$HOME/IA/proyecto-seal/messages"
ALERT_FILE="$MESSAGES_DIR/ada_soul_alert.jsonl"
CHECK_SCRIPT="$MESSAGES_DIR/soul_health_check.sh"
LOCK="/tmp/seal_ada_soul_health.lock"
