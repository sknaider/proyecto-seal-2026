...
async def pending_for_redelivery(conn, retry_after_sec: int = DEFAULT_RETRY_SEC,
                                 limit: int = 100,
                                 solo_agentes: set | None = None) -> list:
    ...
    `solo_agentes` FILTRA EN EL SQL, y existe por un candado circular medido el
    4-sep-2026 04:25: el consumidor pedía las 100 más viejas y descartaba EN
    PYTHON las de agentes fuera del rollout. Como esas nunca se acusaban, seguían
    siendo las más viejas y ocupaban la ventana entera para siempre:

        ventana real de 100 -> ADA 82 · ALICE 15 · FABLE 3
        procesables por el consumidor -> 0

    El tick corría cada 15 s sin entregar ni acusar NADA, y el ledger llevaba sin
    drenarse desde el 25-jun. Filtrar después del LIMIT es pedir las primeras 100
    de una lista y descartarlas todas: hay que filtrar ANTES de limitar.

    Sin `solo_agentes` el comportamiento es el histórico (no filtra), para no
    romper a los otros llamadores.

    ORDEN: lo NUNCA entregado va primero, antes que cualquier reintento. Medido
    el 4-sep 05:15, tras arreglar el filtro: la ventana volvia a llenarse sola,
    esta vez con 100 REINTENTOS de filas ya entregadas y CERO nunca-entregadas.
    Con `ORDER BY priority, created_at` un mensaje que nunca llego pierde contra
    un reintento mas viejo, y los 20 DMs pendientes desde el 31-jul quedaban
    afuera otra vez. Un mensaje no entregado no puede perder contra uno que ya
    llego: `(delivered_at IS NOT NULL) ASC` pone los NULL primero.
    """
    if solo_agentes:
        return await conn.fetch(
            """SELECT msg_id, to_agent, channel, payload, priority, attempts
               ...
               ORDER BY priority ASC, (delivered_at IS NOT NULL) ASC, created_at ASC
               LIMIT $3""",
            retry_after_sec, MAX_ATTEMPTS, limit,
            [str(a).upper() for a in solo_agentes],
        )
    return await conn.fetch(
        """SELECT msg_id, to_agent, channel, payload, priority, attempts
           FROM soul_v3.message_outbox
