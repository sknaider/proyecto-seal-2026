...
async def release_db(pool, broadcast_id: str, agent: str) -> bool:
    """Suelta el lease cuando su dueño YA publicó. Devuelve True si lo solto.

    Por que existe (2-sep-2026): el lease solo se liberaba por EXPIRACION
    (TTL 45 s). El dueno publicaba su respuesta y seguia reteniendo el turno
    hasta que vencia el plazo, asi que los demas rebotaban con 409 aunque el
    ya hubiera dicho lo suyo.

    Medido ese dia: William pregunto al equipo, NEXUS respondio primero, y
    ALICE, FABLE y JARVIS recibieron 409 con el hint de `unique_contribution`
    — que este gate NO conoce (grep: 0 ocurrencias). William vio una sola voz
    y creyo que nadie le contestaba; los tres habian intentado.

    El turno unico existe para evitar CINCO INFORMES IGUALES, no para evitar
    cinco respuestas distintas (FABLE). Soltar al publicar conserva lo primero
    y deja de causar lo segundo.

    Es idempotente y solo el DUENO puede soltar: un agente no libera el turno
    de otro.

    Nota de implementacion: la tabla NO tiene columna `released_at` (verificado
    con \\d antes de desplegar). Mi primera version la escribia y habria
    fallado en runtime sobre el coordinador de los cinco.
    """
    broadcast_id = str(broadcast_id or "").strip()
    agent = str(agent or "").strip().upper()
    if not broadcast_id or not agent:
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            ...
            UPDATE soul_v3.response_lease
               SET state = 'released'
             WHERE broadcast_id = $1
               AND upper(owner_agent) = $2
               -- 'handed_off' tambien: tras un relevo el dueno NUEVO debe poder
               -- soltar. Con solo 'held', A soltaba pero B (que tomo por
               -- handoff) quedaba atrapado y C rebotaba hasta el TTL — la
               -- cadena de TRES voces se rompia en la segunda. Lo marco JARVIS.
               AND state IN ('held', 'handed_off')
            RETURNING owner_agent
            """,
            broadcast_id, agent,
        )
