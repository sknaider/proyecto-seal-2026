#!/usr/bin/env python3
"""ADA DM Poller — polls DB for new DM messages from William to ADA."""
import asyncio
import asyncpg
import json
...
async def poll_dm():
    ...
    print(f"[ADA-DM] Poller iniciado, desde {last_ts}", flush=True)
    ...
    while True:
        try:
            ...
            for r in rows:
                ...
                print(f"[ADA-DM] William: {r['content'][:80]}", flush=True)
            if rows:
                save_last_ts(last_ts)
        except Exception as e:
            print(f"[ADA-DM] error: {e}", flush=True)
            try:
                conn = await asyncpg.connect(DSN)
            except Exception:
                pass
