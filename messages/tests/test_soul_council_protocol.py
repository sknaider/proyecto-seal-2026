...
# El modo CASCADA (William, 2-sep) SUSTITUYE a `discussion` cuando está activo:
# la misma pregunta al equipo, contestada en orden en vez de por un lead solo.
# Se deriva del flag en vez de cablear un literal para que el test siga midiendo
# el clasificador y no el estado del archivo de control — si mañana el flag se
# apaga, este test debe seguir siendo correcto sin editarlo.
_DISCUSION = "cascade" if council.cascade_enabled() else "discussion"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("que opinan de esta idea?", "equipo", _DISCUSION),
        ("no ejecuten nada; solo quiero saber que opinan", "equipo", _DISCUSION),
