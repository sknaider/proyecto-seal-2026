...
def test_dm_busy_ack_defaults_preserve_one_voice_and_visible_liveness():
    assert bridge.DM_LIVE_ACK_ENABLED is True
    assert bridge.DM_BUSY_ACK_GRACE_SECONDS == 10.0
...
def test_busy_terminal_durably_acks_william_without_consuming_dm(monkeypatch):
    ...
    class FakeConn:
        ...
        async def fetchval(self, query, source_ids):
            assert source_ids == ["116921", "db_116921"]
            return None
...
def test_busy_terminal_gives_quick_dm_time_to_receive_one_real_answer(monkeypatch):
    class FakeConn:
        async def fetch(self, query, last_id):
            return [{
                "id": 116921,
        ...
        async def fetchval(self, *_):
            raise AssertionError("a recent DM must wait before checking/posting an ACK")
    ...
    monkeypatch.setattr(
        bridge,
        "post_message",
        lambda *_: (_ for _ in ()).throw(AssertionError("must not post an early ACK")),
    )
...
def test_busy_terminal_does_not_ack_after_any_existing_reply(monkeypatch):
    class FakeConn:
        ...
        async def fetchval(self, query, source_ids):
            assert "db_116921" in source_ids
            return 116999

    monkeypatch.setattr(bridge, "LIVE_ACK_ENABLED", True)
    monkeypatch.setattr(bridge, "DM_LIVE_ACK_ENABLED", True)
    monkeypatch.setattr(bridge, "DM_BUSY_ACK_GRACE_SECONDS", 10.0)
    monkeypatch.setattr(bridge, "read_dm_ack_id", lambda: 116919)
    monkeypatch.setattr(
        bridge,
        "post_message",
...
def test_response_source_ids_include_api_numeric_and_db_spellings():
    msg = bridge.ChatMessage(
        ...
        channel="dm:ada:william",
        message_type="conversation",
        metadata={"legacy_id": "api_william_116921"},
    )

    assert bridge.response_source_ids(msg) == [
        "116921",
        "api_william_116921",
        "db_116921",
    ]
