#!/usr/bin/env python3
"""Provision the review-only SUIE login without printing password material."""
...
async def provision() -> None:
    ...
    if len(token) < 32 or TOKEN_PATH.lstat().st_mode & 0o077 or TOKEN_PATH.is_symlink():
        raise RuntimeError("SUIE review token file is invalid or too broadly readable")
    password = hashlib.sha256(b"seal-suie-review-db-v1\0" + token.encode("utf-8")).hexdigest()
    connection = await asyncpg.connect(bootstrap_dsn)
    try:
        statement = await connection.fetchval(
            "SELECT format('ALTER ROLE svc_soul_ingestion_review LOGIN PASSWORD %L', $1::text)",
            password,
        )
