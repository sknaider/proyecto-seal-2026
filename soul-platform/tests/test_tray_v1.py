...
def test_start_existing_installs_and_activates_managed_descriptor(tmp_path, monkeypatch):
    ...
    calls = []
    monkeypatch.setattr(
        "soul_platform.tray.install_descriptor",
        lambda *args, **kwargs: calls.append(("install", args, kwargs)),
    )
    monkeypatch.setattr(
        "soul_platform.tray.activate_descriptor",
        lambda *args, **kwargs: calls.append(("activate", args, kwargs)),
    )
    monkeypatch.setattr(
        SoulTrayController,
        "status",
...
def test_start_rolls_back_brain_when_activation_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("soul_platform.tray.install_descriptor", lambda *_a, **_k: None)
    monkeypatch.setattr(
        "soul_platform.tray.activate_descriptor",
...
def test_windows_tray_autostart_uses_limited_current_user_task_and_stdin(
    ...
    assert "if($old){Stop-ScheduledTask" in register
