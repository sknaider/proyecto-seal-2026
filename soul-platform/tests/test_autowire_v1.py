...
def test_reconcile_is_idempotent_keeps_current_and_blocks_unattested_memory(
    ...
    assert rows[ollama.provider_id]["memory_allowed"] == 0
...
def test_activation_preserves_embedding_and_uses_generation_fence(
    ...
    manager = AutoWireManager(result.root)
    manager.reconcile()
    # Activation is an internal future seam.  Raw discovery never grants this;
    # the test explicitly models a stronger platform attestor doing so.
    manager.registry.upsert(
        new, state=ProviderState.IDENTITY_ATTESTED, memory_allowed=True
    )
...
def test_raw_http_discovery_never_grants_private_memory(tmp_path, monkeypatch):
    result = _initialized(tmp_path)
    current = _candidate()
    monkeypatch.setattr(
        "soul_platform.autowire.manager.discover_all", lambda: ([current], {})
