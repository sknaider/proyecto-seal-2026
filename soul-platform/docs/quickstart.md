...
tener un **alma persistente**: una identidad + memoria que vive en la máquina.
AutoWire detecta cerebros locales sin entregarles recuerdos por el puerto HTTP;
las aplicaciones compatibles reciben el alma mediante un conector autenticado.
Cambiás el cerebro cuando quieras; el alma y los recuerdos quedan.
...
> **Alcance de Platform 0.5 / proxy v1 (honesto):** solo cerebros **locales**.
...
> **Nota de disponibilidad:** `soul-platform 0.5.0` y `soul-framework 0.4.2`
...
En Platform 0.5.0 el instalador también configura AutoWire en modo `shadow`.
...
Tus aplicaciones pueden usar el **proxy**
(`http://127.0.0.1:11435/v1`) en vez de Ollama directo para mantener una URL de
cerebro estable. En 0.5 esa ruta HTTP es deliberadamente *brain-only*: Codex y
Claude recuperan identidad y recuerdos por el MCP local ligado a su proceso.
El cliente HTTP manda el token así:
...
1. En Codex o Claude, el conector MCP `soul-local` guarda un hecho explícito con
   `soul_memory_store`. Una pregunta nunca se vuelve un hecho por sí sola.
2. Cambiá de cerebro con `switch-brain` (o reiniciá la máquina).
3. Reiniciá el cliente y consultá con `soul_memory_search`: **lo sigue sabiendo.**
   El cerebro cambió; el alma no.
